...
gulp.task("usemin-replace", async function () {
  return gulp
    ...
    .pipe(
      replace(
        /(\.(png|jpg|jpeg|tiff))+(?<!(favicon\.png|app_icon\.png))/g,
        ".webp",
      ),
    )
    .pipe(gulp.dest("dist"));
...
gulp.task("move-pwa-imagens", async function () {
  return gulp
    .src("src/assets/img/pwa/**/*")
    .pipe(gulp.dest("dist/assets/img/pwa"));
});
...
gulp.task(
  ...
  gulp.series(
    "clean",
    "static-files",
    parallel("usemin-replace", "convert-webp", "move-pwa-imagens"),
  ),
);
